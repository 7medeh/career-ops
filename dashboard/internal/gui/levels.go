...
func FetchCompanyProfile(client *http.Client, company string) (CompanyProfile, fetchStatus) {
...
func inferStageFromSize(c levelsCompany) (CompanyStage, bool) {
	n := c.EmpCount
	if n <= 0 {
	...
	case n < 200:
		return StageSeriesB, true
	case n < 500:
		return StageSeriesC, true
	default:
		return StageGrowth, true
	}
}
...
func LoadCompanyProfiles(careerOpsPath string) map[string]CompanyProfile {
	...
	for _, line := range strings.Split(string(b), "\n") {
		...
		out[strings.ToLower(company)] = CompanyProfile{
			Company:        company,
...
func SaveCompanyProfiles(careerOpsPath string, profiles map[string]CompanyProfile) error {
...
func sortedKeys(m map[string]CompanyProfile) []string {
...
var stageTokens = map[CompanyStage]string{
	StageUnknown: "unknown",
	StageSeed:    "seed",
	StageSeriesA: "series_a",
	StageSeriesB: "series_b",
	StageSeriesC: "series_c",
	StageGrowth:  "growth",
	StagePublic:  "public",
}

func stageToken(s CompanyStage) string {
...
func parseStage(s string) CompanyStage {
	s = strings.ToLower(strings.TrimSpace(s))
...
func profileStale(p CompanyProfile) bool {
