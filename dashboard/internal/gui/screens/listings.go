...
type ListingsModel struct {
	listings      []gui.JobListing
	contacts      map[string][]gui.Contact
	status        string
	cursor        int
	scrollOffset  int
	width, height int
...
func NewListingsModel(t theme.Theme, listings []gui.JobListing, careerOpsPath string, width, height int) ListingsModel {
	sorted := make([]gui.JobListing, len(listings))
	...
	return ListingsModel{
		listings:      sorted,
		contacts:      gui.LoadContacts(careerOpsPath),
		width:         width,
		height:        height,
		theme:         t,
...
func (m ListingsModel) Listings() []gui.JobListing {
	return m.listings
}
...
func (m ListingsModel) CurrentListing() (gui.JobListing, bool) {
	if m.cursor < 0 || m.cursor >= len(m.listings) {
		return gui.JobListing{}, false
	}
...
func (m ListingsModel) handleKey(msg tea.KeyMsg) (ListingsModel, tea.Cmd) {
	...
	case "e":
		if l, ok := m.CurrentListing(); ok && l.Company != "" {
			contacts := gui.ContactsFor(m.contacts, l.Company)
			return m, func() tea.Msg {
...
func (m ListingsModel) renderListingLine(l gui.JobListing, selected bool) string {
...
func stageLabel(l gui.JobListing) string {
	s := l.Stage.String()
	if s == "" {
		return "—"
	}
...
func headcountLabel(l gui.JobListing) string {
	if l.Headcount > 0 {
