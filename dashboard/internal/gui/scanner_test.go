...
func TestComputeFit(t *testing.T) {
	...
	strong := JobListing{Title: "Full Stack Engineer, AI", YOE: 3, Salary: "$150K - $180K", Location: "Remote"}
	weak := JobListing{Title: "Staff Backend Engineer", YOE: 10, Salary: "$90K", Location: "Dallas, TX"}
...
func TestIsEvaluated(t *testing.T) {
	...
	if !isEvaluated(JobListing{Company: "Anthropic", Title: "Software Engineer, Full-stack"}, evaluated) {
		t.Error("exact company+role should be evaluated")
	}
	if isEvaluated(JobListing{Company: "Anthropic", Title: "Software Engineer, Desktop"}, evaluated) {
		t.Error("different role at same company should not match (only 2-word overlap 'software engineer' but distinct)")
	}
	if isEvaluated(JobListing{Company: "OpenAI", Title: "Software Engineer, Full-stack"}, evaluated) {
