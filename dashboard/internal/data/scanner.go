...
type ScanResult struct {
	...
	NewListings      []model.JobListing `json:"newListings"`
}
...
func Scan(careerOpsPath string) (ScanResult, error) {
	...
	evaluated := ParseApplications(careerOpsPath)

	today := time.Now().Format("2006-01-02")
	var toAdd []model.JobListing
	...
	if result.NewListings == nil {
		result.NewListings = []model.JobListing{}
	}
	return result, nil
}
...
func fetchCompany(company, provider, slug string) ([]model.JobListing, error) {
	switch provider {
	case "greenhouse":
...
func fetchGreenhouse(company, slug string) ([]model.JobListing, error) {
	...
	jobs := make([]model.JobListing, 0, len(payload.Jobs))
	for _, j := range payload.Jobs {
		jobs = append(jobs, model.JobListing{
			URL:      j.AbsoluteURL,
			Company:  company,
			Title:    j.Title,
			Portal:   "Greenhouse",
...
func fetchLever(company, slug string) ([]model.JobListing, error) {
	...
	jobs := make([]model.JobListing, 0, len(postings))
...
func fetchAshby(company, slug string) ([]model.JobListing, error) {
	...
	jobs := make([]model.JobListing, 0, len(payload.Data.JobBoard.JobPostings))
...
func isEvaluated(job model.JobListing, evaluated []model.CareerApplication) bool {
...
func appendToPipeline(careerOpsPath string, listings []model.JobListing) error {
	if len(listings) == 0 {
		return nil
	}
...
func appendToScanHistory(careerOpsPath string, listings []model.JobListing, date string) error {
	if len(listings) == 0 {
		return nil
	}
...
func enrichDetails(listings []model.JobListing) {
	var wg sync.WaitGroup
	sem := make(chan struct{}, 8)
...
func computeFit(job model.JobListing, fp fitProfile, positives []string) int {
	score := 50
