...
func TestIsStartup(t *testing.T) {
	startups := []model.CompanyStage{model.StageSeed, model.StageSeriesA, model.StageSeriesB, model.StageSeriesC}
	for _, s := range startups {
		if !s.IsStartup() {
			t.Errorf("%v should count as a startup", s)
		}
	}
	for _, s := range []model.CompanyStage{model.StageUnknown, model.StageGrowth, model.StagePublic} {
		if s.IsStartup() {
...
func TestCompanyProfilesRoundTrip(t *testing.T) {
	...
	arize := got["arize ai"]
	if arize.Stage != model.StageSeriesB || !arize.StageInferred {
	...
	nf := got["netflix"]
	if nf.Stage != model.StagePublic || nf.StageInferred {
...
func TestApplyCompanyProfiles(t *testing.T) {
	...
	profiles := map[string]model.CompanyProfile{
		"arize ai": {Stage: model.StageSeriesB, StageInferred: true, Headcount: 90, MedianComp: "~$147K", HQ: "Berkeley, CA"},
		"netflix":  {Stage: model.StagePublic, Headcount: 14000, MedianComp: "~$450K", HQ: "Los Gatos, CA"},
	}

	applyCompanyProfiles(listings, profiles)

	if listings[0].Stage != model.StageSeriesB || !listings[0].StageInferred {
	...
	if listings[2].Stage != model.StageUnknown {
...
func TestProfileStale(t *testing.T) {
	if !profileStale(model.CompanyProfile{Fetched: "2020-01-01"}) {
		t.Error("old profile should be stale")
	}
	if !profileStale(model.CompanyProfile{Fetched: ""}) {
		t.Error("unparseable date should be treated as stale")
	}
	if profileStale(model.CompanyProfile{Fetched: nowStamp()}) {
