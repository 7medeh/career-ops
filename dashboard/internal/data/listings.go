...
func ParseListings(careerOpsPath string) []model.JobListing {
...
func applyCompanyProfiles(listings []model.JobListing, profiles map[string]model.CompanyProfile) {
	if len(profiles) == 0 {
		return
	}
...
func ListingCompanies(listings []model.JobListing) []string {
...
func enrichListing(dst *model.JobListing, src model.JobListing) {
	if dst.Salary == "" {
...
func parsePipelineListings(careerOpsPath string) []model.JobListing {
	...
	for _, line := range strings.Split(string(content), "\n") {
		...
		listing := model.JobListing{Source: "pipeline"}
		if len(parts) > 0 {
...
func parseScanHistoryListings(careerOpsPath string) []model.JobListing {
...
func excludeEvaluated(listings []model.JobListing, evaluated []model.CareerApplication) []model.JobListing {
	if len(evaluated) == 0 {
		return listings
	}
