...
This install is English-only. The non-English mode directories (`modes/de/`, `modes/fr/`, `modes/pt/`) have been removed, and `update-system.mjs` no longer pulls them from upstream. Always use the modes in `modes/`.
