...
4. Detect the JD's language → the CV's language (EN by default)
5. Detect the company's location → paper format:
   - US/Canada → `letter`
...
12. Generate the complete HTML from the template + personalized content
13. Read `name` from `config/profile.yml` → normalize to lowercase kebab-case (e.g. "John Doe" → "john-doe") → `{candidate}`
...
- No critical info in the PDF's headers/footers (ATS ignores them)
- UTF-8, selectable text (not rasterized)
- No nested tables
- JD keywords distributed across: Summary (top 5), the first bullet of each role, Skills section

## PDF design

- **Fonts**: Space Grotesk (headings, 600-700) + DM Sans (body, 400-500)
- **Self-hosted fonts**: `fonts/`
- **Header**: name in Space Grotesk 24px bold + a 2px gradient line `linear-gradient(to right, hsl(187,74%,32%), hsl(270,70%,45%))` + contact row
- **Section headers**: Space Grotesk 13px, uppercase, letter-spacing 0.05em, primary cyan
- **Body**: DM Sans 11px, line-height 1.5
- **Company names**: purple accent `hsl(270,70%,45%)`
- **Margins**: 0.6in
...
- JD says "RAG pipelines" and the CV says "LLM workflows with retrieval" → change to "RAG pipeline design and LLM orchestration workflows"
- JD says "MLOps" and the CV says "observability, evals, error handling" → change to "MLOps and observability: evals, error handling, cost monitoring"
- JD says "stakeholder management" and the CV says "collaborated with team" → change to "stakeholder management across engineering, operations, and business"
...
| `{{EXPERIENCE}}` | HTML for each job, with bullets reordered |
